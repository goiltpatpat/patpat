...
Read-only coverage may run in parallel. Writable workers need exclusive paths and the earned-parallelism gates. If those gates fail, run serially and name missing lanes. Inherit the parent model unless the user named models. Do not invent coverage for a silent worker. The parent verifies the integrated result.
