...
6. Form competing hypotheses. Reconcile by date, scope, author proximity, and observed repository behavior. Label each material conclusion `confirmed`, `inferred`, or `unknown`. Every intent claim cites a commit, pull request, comment, or file:line, or is labeled inferred or hypothesis. Repo-scoped only.
