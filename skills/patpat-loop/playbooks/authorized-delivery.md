...
1. Name the remaining action: merge, publish, or deploy. Default commit and pull request already ran unless the user opted out.
2. Inspect the live head, CI, and the PR URL. Require current verify and review receipts for that head.
3. Overnight, going to bed, don't stop, land this, merge this, or ship it is enough to merge a green, verified PR. State-then-wait still applies when the user asked only for the plan.
