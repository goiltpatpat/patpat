...
3. Do not reconstruct from transcripts alone. Reconstruct the objective, intended changes, decisions, failed attempts, proof claims, and remaining actions from the smallest available evidence set.
...
Lock the time window before mining (default last 7 days). Never quietly turn all into recent N. Mine only in-scope workspace transcripts. Never read another project's transcripts without being asked. When the user asks, use Patpat-native `agent-transcripts/`. Skip mining if the user already gave a full state capsule.
