...
Use [`patpat-ship/scripts/pr_watch.py`](../../patpat-ship/scripts/pr_watch.py) when the host can provide trusted PR observations. On GitHub.com, capture that input with [`patpat-ship/scripts/github_observe.py`](../../patpat-ship/scripts/github_observe.py); it pins `gh api graphql` to `github.com`, uses a static read-only query, and fails closed on truncated review threads or checks. Otherwise generate the schema-v3 shape with `python3 skills/patpat-ship/scripts/pr_watch.py --print-example`. Bind every observation to repository, PR number, full head SHA, and base branch. Read required check names from a verified repository policy or CI contract. The GitHub observer rejects an empty required-check list unless `--allow-no-required-checks` explicitly records that CI is not required; never use the override merely to reach `ready`. Set a bounded maximum observation age; the evaluator compares it with its own UTC clock and rejects replayed evidence or timestamps beyond the bounded clock skew. Keep credentials, wake control, and provider mutations outside the evaluator.
