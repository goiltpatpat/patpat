...
For a revision-bound Codex contract canary, run [`../../scripts/probe_codex_behavior.py`](../../scripts/probe_codex_behavior.py) manually against a clean committed revision and an explicit requested model. Keep the generated JSONL and receipt outside every Git worktree. The probe checks bounded task behavior, observed mutation-capable commands, and response-shape conformance; it does not prove host-attested skill activation, every route, the resolved provider snapshot, or every possible transient side effect. Its result applies only to the recorded Codex version, requested model selection, and Patpat revision. Do not transfer it to Cursor, Grok, Antigravity, or a later commit.
